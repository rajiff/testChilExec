...
let executeCMD = function(cmd, input, callback) {
  ...
  let cmdProcess = child_process.spawn(path.resolve(__dirname, cmd),
    {env: _.merge(input, process.env)}, {detached: false});
  ...
  cmdProcess.stderr.on('message', (data) => {
    console.log(cmd,' MESSAGE:', data.toString());
    stderr += data.toString();
  });
  ...
  cmdProcess.on('exit', (exitCode) => {
    cmdProcess = null;
    console.log('Process (', cmd, ') exited code:', exitCode);
    clearTimeOutHandler();
    callback(null, {stdout, stderr, exitCode});
  });
  ...
  cmdProcess.on('error', (err) => {
    cmdProcess = null;
    console.log('Process (', cmd, ') got error:', err);
    stderr += err;
    exitCode = -1;
    clearTimeOutHandler();
    callback(null, {stdout, stderr, exitCode});
  ...
  clearTimeOutHandler = () => {
    if(timeOutHandler) {
      console.log('Clearing timeout event explicitly');
      clearTimeout(timeOutHandler);
    }
  }
};

executeCMD('run', {}, function(err, results){
  console.log('[*****] Command execution completed with results: ', results);
});
